...
const showEpubFileOrFolderBrowseDialog = async (callback) => {
    console.log("main process file or folder dialog")
  const filePath = await showOpenDialog({
...
function setupFileDialogEvents() {
  ...
  // comes from the renderer process (ipcRenderer.send())
  ipcMain.on(IPC_EVENT_showEpubFileOrFolderBrowseDialog, async (event, payload) => {
    console.log("hi from IPC_EVENT received by main")
